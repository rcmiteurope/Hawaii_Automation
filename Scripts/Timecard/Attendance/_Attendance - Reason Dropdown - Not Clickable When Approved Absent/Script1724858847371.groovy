...
for (int dow = 1; dow <= 5; dow++) {
	...
	if (numberOfStudents != 0) {
		...
		for (int i = 0; i < numberOfStudents; i++) {
			...
			// if is approved *and* is present
			if (CustomKeywords.'timecard.isApproved'(dayOfWeek) 
				&& CustomKeywords.'timecard.isAbsent'()) {
				
				WebElement weReasonDropdown = driver.findElement(By.xpath('//*[@id="root"]/div/main/main/div[4]/div/div[2]/select'))
//	ID missing	WebElement weReasonDropdown = driver.findElement(By.xpath('//*[@id="????????"]'))
				TestObject toReasonDropdown = WebUI.convertWebElementToTestObject(weReasonDropdown)

				// found testable students
				found = true

				// verify reason dropdown is not clickable
				WebUI.verifyElementNotClickable(toReasonDropdown)
