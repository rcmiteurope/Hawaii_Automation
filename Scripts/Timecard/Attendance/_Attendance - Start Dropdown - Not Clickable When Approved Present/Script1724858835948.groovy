	...
	if (numberOfStudents != 0) {
		...
		for (int i = 0; i < numberOfStudents; i++) {
			...
			if (CustomKeywords.'timecard.isApproved'(dayOfWeek) 
				&& CustomKeywords.'timecard.isPresent'()) {
				
				WebElement weStartDropdown = driver.findElement(By.xpath('//*[@id="btn-TimeIn"]'))
				TestObject toStartDropdown = WebUI.convertWebElementToTestObject(weStartDropdown)

				// found testable students
				found = true

				// verify start dropdown is not clickable
				WebUI.verifyElementNotClickable(toStartDropdown)
