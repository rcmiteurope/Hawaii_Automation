	...
	if (numberOfStudents != 0) {
		...
		for (int i = 0; i < numberOfStudents; i++) {
			...
			if (CustomKeywords.'timecard.isNotApproved'(dayOfWeek)) {
				...
				// setup for absent
				CustomKeywords.'timecard.setToAbsent'()
				
				// set absent reason to blank
				CustomKeywords.'timecard.setAbsentReason'("Select...")
	
				// click the present button
				driver.findElement(By.xpath(('//*[@id="presentBtn"]'))).click()
	
				// verify no warning messagebox visible
				WebUI.verifyTextNotPresent('Are you sure?', false)
