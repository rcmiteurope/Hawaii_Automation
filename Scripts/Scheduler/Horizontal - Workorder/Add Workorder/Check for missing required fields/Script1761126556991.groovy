...
WebUI.click(search)
WebUI.delay(1)
//WebUI.setText(search, 'Tyler Abe')
//WebUI.selectOptionByIndex(search, 1)
WebUI.sendKeys(search, Keys.chord(Keys.ENTER))
WebUI.delay(1)
WebUI.click(addWorkorderBtn)

//WebUI.setText(inputSchool, 'Ahuimanu')
...
// Step: Expect an error message for invalid date range
TestObject invalidDateError = new TestObject('invalidDateError')
invalidDateError.addProperty('xpath', ConditionType.EQUALS, "//div[contains(text(),'Enter End Date earlier than Start Date')]")

// Wait up to 5 seconds for the error message to appear
boolean isErrorDisplayed = WebUI.waitForElementVisible(invalidDateError, 5)

if (isErrorDisplayed) {
	KeywordUtil.markPassed("✅ Correctly handled invalid date range — error message displayed.")
} else {
	KeywordUtil.markFailed("❌ No error message appeared for invalid date range.")
}
