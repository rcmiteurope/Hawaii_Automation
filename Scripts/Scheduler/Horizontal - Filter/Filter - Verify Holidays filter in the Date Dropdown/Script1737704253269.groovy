...
WebElement selectedOption = dropdown.findElement(By.xpath("//option[text()='Holidays']"))
...
for (WebElement cell : tableCells) {
    ...
    Assert.assertTrue(cellText.contains("Holiday"), "Cell does not contain 'Holiday': " + cellText)

    Assert.assertEquals(backgroundColor, "rgba(0, 0, 0, 0)", "Cell does not have a black background: " + backgroundColor)
}
