...
try {
    def results = sql.rows("SELECT ROUTINE_NAME FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_TYPE = 'PROCEDURE' AND ROUTINE_SCHEMA = 'client_services_dev' ORDER BY ROUTINE_NAME;")
    results.each { row ->
        actualSPs.add(row.ROUTINE_NAME)
    }
    
    // Print all stored procedures found in the database
    KeywordUtil.logInfo("Stored Procedures found in Dev DB: " + actualSPs)
...
if (missingSPs.isEmpty() && extraSPs.isEmpty()) {
   ...
   String errorMessage = ""
    ...
    if (!missingSPs.isEmpty()) {
        errorMessage += "Missing Stored Procedures in Dev: " + missingSPs + "\n"
    }
    if (!extraSPs.isEmpty()) {
        errorMessage += "Extra Stored Procedures Found in Dev DB: " + extraSPs + "\n"
    }
