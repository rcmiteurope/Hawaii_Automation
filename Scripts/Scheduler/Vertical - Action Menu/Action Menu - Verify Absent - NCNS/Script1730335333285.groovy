...
try {
	...
	if (!rows.isEmpty()) {
		...
		if (newCellValue.contains("Student Absent - NCNS")) {
    WebUI.comment("Validation passed: The value contains 'Student Absent NCNS'.")
} else {
    WebUI.comment("Validation failed: The value does not contain 'Student Absent NCNS'. Found: " + newCellValue)
}
	} else {
